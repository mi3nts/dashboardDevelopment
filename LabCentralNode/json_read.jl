...
Npoints = 1000
basepath = "/home/teamlary/mintsData/rawMQTT/001e06373996"


sensors = Dict(:IPS7100 => Dict(:path => joinpath(basepath, "IPS7100.json"),
                                :dt => DateTime[],
                                :pm0_1 => Float64[],
                                :pm0_3 => Float64[],
                                :pm0_5 => Float64[],
               ...
               :APDS9002 => Dict(:path => joinpath(basepath, "APDS9002.json"),
                                 :luminance => Float64[],
                                 :dt => DateTime[],
                                 ),
...
function IPS7100()
    res = JSON.parsefile(sensors[:IPS7100][:path])
    ...
    if length(sensors[:IPS7100][:dt]) > 0 &&  dt == sensors[:IPS7100][:dt][end]
        return -1
    end


    if length(sensors[:IPS7100][:dt]) < Npoints
        push!(sensors[:IPS7100][:dt], dt)
    else
        sensors[:IPS7100][:dt] .= circshift(sensors[:IPS7100][:dt], -1)
        sensors[:IPS7100][:dt][end] = dt
    end




    if length(sensors[:IPS7100][:pm0_1]) < Npoints
        push!(sensors[:IPS7100][:pm0_1], parse(Float64, res["pm0_1"]))
    else
        sensors[:IPS7100][:pm0_1] .= circshift(sensors[:IPS7100][:pm0_1], -1)
        sensors[:IPS7100][:pm0_1][end] = parse(Float64, res["pm0_1"])
    end


    if length(sensors[:IPS7100][:pm0_3]) < Npoints
        push!(sensors[:IPS7100][:pm0_3], parse(Float64, res["pm0_3"]))
    else
        sensors[:IPS7100][:pm0_3] .= circshift(sensors[:IPS7100][:pm0_3], -1)
        sensors[:IPS7100][:pm0_3][end] = parse(Float64, res["pm0_3"])
    end


    if length(sensors[:IPS7100][:pm0_5]) < Npoints
        push!(sensors[:IPS7100][:pm0_5], parse(Float64, res["pm0_5"]))
    else
        sensors[:IPS7100][:pm0_5] .= circshift(sensors[:IPS7100][:pm0_5], -1)
        sensors[:IPS7100][:pm0_5][end] = parse(Float64, res["pm0_5"])
    end


    if length(sensors[:IPS7100][:pm1_0]) < Npoints
        push!(sensors[:IPS7100][:pm1_0], parse(Float64, res["pm1_0"]))
    else
        sensors[:IPS7100][:pm1_0] .= circshift(sensors[:IPS7100][:pm1_0], -1)
        sensors[:IPS7100][:pm1_0][end] = parse(Float64, res["pm1_0"])
    end


    if length(sensors[:IPS7100][:pm2_5]) < Npoints
        push!(sensors[:IPS7100][:pm2_5], parse(Float64, res["pm2_5"]))
    else
        sensors[:IPS7100][:pm2_5] .= circshift(sensors[:IPS7100][:pm2_5], -1)
        sensors[:IPS7100][:pm2_5][end] = parse(Float64, res["pm2_5"])
    end


    if length(sensors[:IPS7100][:pm10_0]) < Npoints
        push!(sensors[:IPS7100][:pm10_0], parse(Float64, res["pm10_0"]))
    else
        sensors[:IPS7100][:pm10_0] .= circshift(sensors[:IPS7100][:pm10_0], -1)
        sensors[:IPS7100][:pm10_0][end] = parse(Float64, res["pm10_0"])
    end
    ...
    if length(sensors[:IPS7100][:bins]) < Npoints
        push!(sensors[:IPS7100][:bins], bins)
    else
        sensors[:IPS7100][:bins] .= circshift(sensors[:IPS7100][:bins], -1)
        sensors[:IPS7100][:bins][end] = bins
    end
