...
pm1_ips = deque(maxlen=l)#pm1 ips
pm2_5_ips = deque(maxlen=l)#pm2.5 ips ips
pm10_ips = deque(maxlen=l)#pm10 ips
...
################################################ HEADER ##################################################
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.H4("CENTRAL NODE DASHBOARD", className="app__header__title"),
                        html.P(
                            " ",
...
############################################## BODY ###################################################################
    ...
    html.Div([   
            ...
            html.Div([
                html.Div(
                    [html.H6("Particulate Matter (IPS7100)", className="graph__title")]
                ),
        		dcc.Graph(id = 'pm-ips-live-graph', animate = True),
        		dcc.Interval(
        			id = 'pm-ips-graph-update',
        			interval = inter,
        			n_intervals = 0
        		),
            ],
    ...
    ########################### Bin IPS #######################            
            html.Div([
                    html.Div(
                        [html.H6("Size Distribution (IPS7100)", className="graph__title")]
                    ),
            		dcc.Graph(id = 'bin-ips-live-graph', animate = True),
            		dcc.Interval(
             			id = 'bin-ips-graph-update',
             			interval = inter,
             			n_intervals = 0
            		),
                ],
        ...
        ############### Contour #################### 
            ...
            html.Div([
                html.Div(
                    [html.H6("Contour Distribution (IPS7100)", className="graph__title")]
                ),
        		dcc.Graph(id = 'contour-ips-live-graph',),# animate = True),
        		dcc.Interval(
        			id = 'contour-ips-graph-update',
        			interval = inter,
        			n_intervals = 0
        		),
            ],
...
def update_graph_wind(n):
    df_json = pd.read_json('mintsData\\rawMQTT\\00044be5fc9d\\WIMWV.json', lines = True)
...
def update_graph_pht(n):
    df_json = pd.read_json('mintsData\\rawMQTT\\001e06373996\\BME280.json', lines = True)
...
def update_graph_pm(n):
    df_json = pd.read_json('mintsData\\rawMQTT\\001e06373996\\OPCN3.json', lines = True)
...
@app.callback(Output('pm-ips-live-graph', 'figure'),[ Input('pm-ips-graph-update', 'n_intervals') ])
def update_graph_pm_ips(n):
    df_json = pd.read_json('mintsData\\rawMQTT\\001e06373996\\IPS7100.json', lines = True)
 
    date_time_pm_ips.append(df_json['dateTime'][0])

    pm1_ips.append(df_json['pm1_0'][0])
    pm2_5_ips.append(df_json['pm2_5'][0])
    pm10_ips.append(df_json['pm10_0'][0])
    # df_wide = pd.DataFrame([list(date_time_pm),list(pm1),list(pm2_5),list(pm10)]).transpose()
    # df_wide.columns = ["dateTime","pm1","pm2_5","pm10"]
    # df_long=pd.melt(df_wide, id_vars=['dateTime'], value_vars=['pm1', 'pm2_5', 'pm10'])
    # fig3 = px.scatter(df_long, x='dateTime', y='value', color='variable',marginal_y="histogram")
    trace0 = go.Scatter(
                x= list(date_time_pm_ips),
                y= list(pm1_ips),
                name='PM 1',
                mode= 'lines+markers'#,marginal_y="histogram"
              )
    ...
    trace3 = go.Histogram(y=list(pm1_ips),name='PM 1',marker_color='rgb(99,110,250)',)
    trace4 = go.Histogram(y=list(pm2_5_ips),name='PM 2.5',marker_color='rgb(239,85,59)')
    trace5 = go.Histogram(y=list(pm10_ips),name='PM 10',marker_color='rgb(0,204,150)')
    
    fig4 = tools.make_subplots(rows = 2, cols =3,
                                specs=[[{"rowspan":2,"colspan":2}, None,{"rowspan":2} ],
                                       [None, None,None]],
                               horizontal_spacing = 0.2 )
    ...
    #fig3=go.Figure(data)
    fig4.append_trace(trace3, 1, 3)
    fig4.append_trace(trace4, 1, 3)
    fig4.append_trace(trace5, 1, 3)
    fig4.update_layout(xaxis = dict(range = [min(date_time_pm_ips),max(date_time_pm_ips)]),
                        xaxis_title="Date Time ",
                        yaxis_title="Particulate Matter (\u03BC"+"g/"+ "m"+"\u00b3"+")",
                        plot_bgcolor=app_color["graph_bg"],
                        paper_bgcolor=app_color["graph_bg"],
...
def update_graph_no2(n):
    df_json = pd.read_json('mintsData\\rawMQTT\\001e06373996\\MGS001.json', lines = True)
...
@app.callback(Output('bin-ips-live-graph', 'figure'),[ Input('bin-ips-graph-update', 'n_intervals') ])
def update_graph_bin_ips(n):
    df_json = pd.read_json('mintsData\\rawMQTT\\001e06373996\\IPS7100.json', lines = True)
    df_json = df_json.iloc[:,1:8]
    bin_ls = list(df_json.iloc[0])
    bin_boundries_high = [0,0.1,0.3,0.5,1.0,2.5,5.0]
    bin_boundries_low  = [0.1,0.3,0.5,1.0,2.5,5.0,10.0]
    ...
    fig6.update_xaxes(rangeslider_visible=True,rangeslider_thickness = 0.05)
    fig6.update_xaxes(gridcolor='#1B2AAA',showline=True, linecolor= '#1B2AAA', mirror=True)
...
def update_graph_contour(n):
    ...
    if(len(df_updated)>1):
        ...
        return fig8
@app.callback(Output('contour-ips-live-graph', 'figure'),[ Input('contour-ips-graph-update', 'n_intervals') ])
def update_graph_contour_ips(n):
    # if file does not exist write header
    # time.sleep(5)
    
    df_json = pd.read_json('mintsData\\rawMQTT\\001e06373996\\IPS7100.json', lines = True) 
    #print(df_json)
    global df_updated_ips
    df_updated_ips = df_updated_ips.append(df_json,ignore_index = True)
    df_updated_ips = df_updated_ips.drop_duplicates()
    #print("SDASDAD",df_updated)
    if(len(df_updated_ips)>1):
        if(len(df_updated_ips)>50):
            df_updated_ips = df_updated_ips.iloc[1:]
        date_time_contour_ips=list(df_updated_ips['dateTime'])
    
        df_bins = df_updated_ips.iloc[:,1:8].T
        df_bins_log = np.log10(df_bins).replace(-np.inf, np.finfo(float).eps)
        list_bins = df_bins_log.values.tolist()
        ...
        bin_boundries_avg_size = list((np.add(bin_boundries_high , bin_boundries_low)/2))  
        print("z",list_bins[0][0:5], len(list_bins[0]))
        print("date",date_time_contour_ips[0:5])
        print("avg",bin_boundries_avg_size[0:2])
        data = go.Figure(data =
            go.Contour(
                z=list_bins,
                x=date_time_contour_ips,
                y=bin_boundries_avg_size, # vertical axis
                contours_showlines=False,
                colorscale = "Jet",
                        colorbar=dict(
        ...
        fig9.update_layout( 
                            yaxis = dict(
                                    #tickmode = 'array',
                                    tickvals = [bin_boundries_avg_size[i] for i in range(0,len(bin_boundries_avg_size))],
        ...
        fig9.update_xaxes(gridcolor='#1B2AAA',tickangle = 45,showline=True, linecolor= '#1B2AAA', mirror=True)
        fig9.update_yaxes(gridcolor='#1B2AAA',showline=True, linecolor= '#1B2AAA', mirror=True,rangemode="tozero")
