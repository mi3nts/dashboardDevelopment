...
otter_path = "mintsData\\rawMQTT\\001e0610c2ed\\"
...
################################################ HEADER ##################################################
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.H4("BOAT DASHBOARD", className="app__header__title"),
                        html.P(
                            " ",
...
def update_graph_pht(n):
    df_json = pd.read_json(otter_path + 'BME280.json', lines = True)
...
def update_graph_pm(n):
    df_json = pd.read_json(otter_path +'OPCN3.json', lines = True)
...
def update_graph_ch4(n):
    df_json = pd.read_json(otter_path + 'MGS001.json', lines = True)
